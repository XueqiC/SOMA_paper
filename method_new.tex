...
SOMA has three stages. The original model \(F\) serves the early turns and thereby establishes the dialogue state. Soft-prompt mining then finds where the surrogate \(G\) is least aligned with \(F\) within that state, and the hard cases it exposes are distilled into a lightweight LoRA adapter. Finally, a semantic gate hands the session to \(G\) once local alignment is sufficient, and a drift detector returns it to \(F\) when the dialogue leaves the learned region.
...
\]
Here $\mathrm{tok}_G(\cdot)$ is the tokenizer of $G$, $\mathbf E(\cdot)$ maps text tokens to embeddings, and $\oplus_{\mathrm{emb}}$ concatenates along the sequence dimension. This construction lets $F$ and $G$ be compared under the same dialogue state while allowing gradients to flow only through $\mathbf P$ on $G$\rev{; the original-model responses used as mining targets are generated once in the warm-start window and kept fixed during optimization, so $F$ is never re-queried while $\mathbf P$ changes (Appendix~\ref{sec:app_rationale})}.
