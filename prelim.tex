...
We describe this challenge through a local manifold view. Given a prefix \(\mathcal{D}_{k}\), the original model \(F\) and the surrogate \(G\) each induce a local region in their contextual representation space, and this region captures the current topic, constraints, style, and task state. The goal is a local rather than a global match, which is what makes a much smaller surrogate viable: the surrogate's region should match that of the original model under the same prefix.
