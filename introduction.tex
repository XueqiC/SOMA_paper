...
\item \textbf{SOMA: Efficient multi-turn serving.} Soft prompts expose where the surrogate diverges most from the original model, a localized LoRA adapter closes that gap, and a cosine gate handles switching and rollback at prompt-free inference time.

\item \textbf{Theory analysis and empirical evaluation.} 
We give concentration bounds for switching, coverage guarantees for prompt-direction search, and suboptimality limits for the selected directions, and we validate SOMA on six multi-turn benchmarks and two model families.
\end{itemize}
