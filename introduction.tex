...
Large language models (LLMs) such as the GPT series~\citep{brown2020language, achiam2023gpt}, LLaMA~\citep{touvron2023llama2, grattafiori2024llama3}, Claude~\citep{anthropic_introducing_claude_2023}, and DeepSeek~\citep{guo2025deepseek} power machine-learning-as-a-service (MLaaS) applications including chat assistants, content creation, coding, and clinical decision support~\citep{park2023thinking, dong2023towards, zhao2025amplifying, liu2024exploring, li2026llmclinicalgraphstructure}, and \textit{multi-turn LLM serving}, which covers extended interactions between humans and LLMs or among LLM agents, has become a key research focus~\citep{yi2024survey, li2025beyond}. Serving such interactions efficiently remains hard: most serving systems are stateless and resend the entire conversation history with every new query~\citep{gao2024cost, jeong2025accelerating}, so computation, latency, and cost all grow as the conversation lengthens.

Prior work follows two routes. \textit{Single-model methods} compress the dialogue history~\citep{wang2025recursively, chen2024compress, xiao2024efficient}, retrieve memory from external modules~\citep{melz2023enhancing, gutierrez2024hipporag}, or reuse attention computation~\citep{gao2024cost, jeong2025accelerating, anthropic_prompt_caching_2024}; they still run the large model at every turn, so cost, latency, and GPU usage stay high, and they often truncate the context that long dialogues depend on. \textit{Multi-model methods} route easy queries to small models and escalate hard ones~\citep{behera2025towards, ding2024hybrid, shnitzer2023large, cheng2026latentrouter}, but small models generalize poorly across dialogue complexity, and switching adds overhead. Moreover, LLMs rely heavily on early turns~\citep{xiao2024efficient, laban2025llms}, which makes coherence harder to maintain as a dialogue grows. 
...
\item \textbf{SOMA: Efficient multi-turn serving.} \rev{Adversarial soft prompts measure how robustly the surrogate reproduces the original model on each early turn, a localized LoRA adapter distills the behavior that holds up,} and a cosine gate handles switching and rollback at prompt-free inference time.

\item \textbf{Theory analysis and empirical evaluation.} 
\rev{We derive a held-out acceptance bound for switching, a per-session cost model, and a coverage bound for random prompt candidates under explicit assumptions,} and we validate SOMA on six multi-turn benchmarks and two model families.
\end{itemize}
